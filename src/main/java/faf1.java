...
public class faf1 {
    ...
    private static void runOneFullCycle(WebDriver driver) {
        driver.get("https://elem.mobi/funnyfights/?autotune=on");
        sleep(3000);

        for (int i = 1; i <= 5; i++) {
            driver.get("https://elem.mobi/funnyfights/enemy/" + i + "/");
            sleep(2500);

            // Language-independent XPath matching attack links
    ...
    private static void collectFreeGemsIfAvailable(WebDriver driver) {
        try {
            driver.get("https://elem.mobi/funnyfights/");
            sleep(2500);
